/*********************************************
   gcc -O1 -fopenmp basic_MM.c -lrt -o basic
   OMP_NUM_THREADS=4 ./basic
*/
...
void smm_simple(matrix_ptr a, matrix_ptr b, matrix_ptr c);
...
int main() {
    ...
    printf("Testing basic MM, parallel, and OMP versions for Sparse Matrices\n");
    ...
    long int data_size = 3 * alloc_size * sizeof(float);
    if(data_size < 32*1024)
        printf("Data fits L1 cache\n");
    else if (data_size< 256*1024)
        printf("Data fits L2 cache\n");
    else if(data_size< 12288*1024)
        printf("Data fits L3 cache");
    else
        printf("Data DOES NOT fit L3 cache");
    ...
    printf("rowlen, smm_basic, smm_parallel, smm_omp \n");
    {
        int i, j;
        for (i = 0; i < x; i++) {
...
void smm_simple(matrix_ptr a, matrix_ptr b, matrix_ptr c) {
    ...
    for (i = 0; i < row_length; i++) { //rows in A
        for (j = 0; j < row_length; j++) { // columns in B
            sum = 0;
            for (k = 0; k < row_length; k++)
                if(a0[i*row_length+k]!=0 && b0[k*row_length+j]!=0)
 ...
 * each thread gets n/NUM_THREADS rows from A
 * multiplies that with B
 * gets n/NUM_THREADS rows of C
...
void *smm_work(void *threadarg){
    ...
    for (i = low; i < high; i++) { //rows in A
        for (j = 0; j < rowlen; j++) { // columns in B
            sum = 0;
            for (k = 0; k < rowlen; k++)
                if(aM[i*rowlen+k]!=0 && bM[k*rowlen+j]!=0)
                    sum += aM[i*rowlen+k] * bM[k*rowlen+j];
            cM[i*rowlen+j] += sum;
        }
    }
    pthread_exit(NULL);
...
/* OMP code gives a baseline for the parallel code.*/
void smm_omp(matrix_ptr a, matrix_ptr b, matrix_ptr c){
    long int i, j, k;
    long int row_length = get_matrix_rowlen(a);
    data_t *a0 = get_matrix_start(a);
